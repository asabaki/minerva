...
router.get('/test/create/activity_tracker', async (req, res) => {
  try {
    const users = await User.find();
    users.forEach(async user => {
      const Tracker = await UserActivity.findOne({user_id: user._id});
      if (!Tracker) {
        const createTracker = await UserActivity.create({
          user_id: user._id
        });
